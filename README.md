...
※ 既存プロジェクトを更新する場合も `supabase/schema.sql` を再実行してください（`threads` の追加カラム `has_episodes` / `episode_number_start`、`subitem_templates` / `subitem_tag_presets` テーブル、`speaker_profiles.speech_balloon_id` 列を作成します）。
