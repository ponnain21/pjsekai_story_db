...
※ 既存プロジェクトを更新する場合も `supabase/schema.sql` を再実行してください（`threads` の追加カラムと `subitem_templates` テーブルを作成します）。
...
Google ログイン後に `項目 -> 項目内項目（日付/タグ）` の順で登録できます。本文は項目内項目を選択して入力します。項目内項目はテンプレート保存、並べ替え、削除ができます。
